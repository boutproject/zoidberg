...
def gen_newr(r, z, splitedlist, bounds):
    ...
    for alist in splitedlist:
        ...
        A=np.array([[1,0.5,1/300,0,0,0],[0,0,0,1,0.5,1/300],[diff_minus_z,0,0,-diff_minus_r,0,0],[diff_plus_z,diff_plus_z,diff_plus_z/100,-diff_plus_r,-diff_plus_r,-diff_plus_r/100]])
        b=np.array([[r[end_index]-r[start_index]],[z[end_index]-z[start_index]],[0],[0]]) 
        x=linalg.lstsq(A,b)
        S=np.linspace(0,1,num=len(alist))
        X=r[start_index]+x[0][0]*S+(x[0][1]*S**2)*0.5+(x[0][2]*S**3)/300
        Y=z[start_index]+x[0][3]*S+(x[0][4]*S**2)*0.5+(x[0][5]*S**3)/300
        if start_index<0:
