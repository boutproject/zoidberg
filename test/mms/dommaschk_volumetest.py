...
def dommaschk_grid_volume(nx,ny,nz,a1,a2,R0,Btor,symmetry, plotting = False):
    ...
    C[5,2,1] = -1.489
    C[5,2,2] = -1.489
    field = zb.field.DommaschkPotentials(C, R_0=R0, B_0=Btor)
    y_grid = np.linspace(0.0, yperiod, ny, endpoint=False)

    rzcoord, _ = zb.fieldtracer.trace_poincare(field, (a1,a2), 0.0, 2.0*np.pi/symmetry,y_slices=y_grid,
                                      revs=200,nplot=1,nover=20)
    inner_lines = []
    outer_lines = []
    pol_grids = []
    for i in range(ny):
        
        inner_line = zb.rzline.line_from_points(rzcoord[:,i,0,0],
                                                      rzcoord[:,i,0,1]
                                                      ,spline_order=1)
        outer_line = zb.rzline.line_from_points(rzcoord[:,i,1,0],
                                                      rzcoord[:,i,1,1]
                                                      ,spline_order=1)
        ...
        pol_grid = zb.poloidal_grid.grid_elliptic(inner_lines[i], outer_lines[i],
                                                        nx, nz, restrict_size=2560,
                                                        align=0, inner_ort=1,
                                                      inner_maxmode=4, nx_inner=0,nx_outer=0)
    ...
    grid = zb.grid.Grid(pol_grids, y_grid, 2.0*np.pi/symmetry, yperiodic=True)
    maps = zb.make_maps(grid, field,nslice=1,num = 10)

    filename = os.path.join(script_dir, f"dommaschk_testgrid_{nx}_{ny}_{nz}.nc")
    zb.write_maps(grid, field, maps,metric2d=False,gridfile=filename)
...
def calculate_dommaschk_volume(ny,revs,symmetry,yperiod,plotting,
                               Btor, R0, a1, a2, mode=1):
    C = np.zeros((6, 3, 4))
    ...
    all_outer_poly = []
    for i in range(ny):
        interp_inner = scipy.interpolate.interp1d(inner_theta[:,i],inner_radius[:,i],
                                                  fill_value="extrapolate",kind="linear")
        interp_outer = scipy.interpolate.interp1d(outer_theta[:,i],outer_radius[:,i],
                                                  fill_value="extrapolate",kind="linear")
        ...
        if plotting:
            ...
            ax.scatter(inner_R[:,i], inner_Z[:,i],edgecolor="None",color="black",s=2.0)
            ax.scatter(outer_R[:,i], outer_Z[:,i],edgecolor="None",color="black",s=2.0)
            ax.scatter(inner_newR[:,i], inner_newZ[:,i],edgecolor="None",color="red",s=1.0)
            ax.scatter(outer_newR[:,i], outer_newZ[:,i],edgecolor="None",color="red",s=1.0)
            ax.set_aspect("equal")
            ax.grid(True)
            ax.set_ylim(-0.4,0.4)
            ax.set_xlim(1.7,2.3)
    ...
    outer_volume = torus_volume_from_sections_periodic(all_outer_poly,y_grid,symmetry, mode=mode)
    inner_volume = torus_volume_from_sections_periodic(all_inner_poly,y_grid,symmetry, mode=mode)
...
if __name__ == "__main__":
    ...
    exact_volume = calculate_dommaschk_volume(2000,500,symmetry,yperiod,plotting,
                                   Btor, R0, a1, a2,mode = 1)
    ...
    for i in range(len(scales)):
        ...
        nx = 8*scale
        nz = 32*scale
        gridvolumes[i] = dommaschk_grid_volume(nx,ny,nz,a1,a2,R0,Btor,symmetry,plotting=plotting)
